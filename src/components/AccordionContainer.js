...
const AccordionContainer = () => {
  ...
  return (
    <Accordion
      className="py-6 transition-all ease-in-out duration-300"
      index={indices}
      onChange={toggleItem}
    >
      {accordionData.map((data, index) => (
        ...
        >
          <AccordionButton className="border-b border-white py-3 w-full flex">
          ...
          <AccordionPanel className="pl-2">{data.description}</AccordionPanel>
        </AccordionItem>
      ))}
    </Accordion>
  )
